...
export async function getCNFTArgs(
  rpc: Rpc<SolanaRpcApi>,
  mint: Address,
  assetFields: DAS.GetAssetResponse,
  proofFields: DAS.GetAssetProofResponse
): Promise<CNFTInstructionArgs> {
  ...
  const creatorShares = assetFields.creators
    ? Uint8Array.from(assetFields.creators.map((c: DAS.Creators) => c.share))
    : new Uint8Array();
  const creatorVerified = assetFields.creators
    ? assetFields.creators.map((c: DAS.Creators) => c.verified)
    : [];
  const sellerFeeBasisPoints = assetFields.royalty
  ...
  const creatorHash = new Uint8Array(
    getAddressEncoder().encode(address(assetFields.compression.creator_hash))
  );

  // get creators (used as remaining accounts)
  const creators: Array<[Address, number]> | undefined =
    assetFields.creators?.map((c: DAS.Creators) => [
      address(c.address),
      c.share,
    ]);

  // 3. get common cNFT transfer fields
  const { root, merkleTree, proof, index, tMetadataArgsArgs, canopyDepth } =
    await getCNFTTransferArgs(rpc, assetFields, proofFields);
  return {
    metaHash,
    creatorShares,
    creatorVerified,
...
export async function getCNFTTransferArgs(
  rpc: Rpc<SolanaRpcApi>,
  assetFields: DAS.GetAssetResponse,
  proofFields: DAS.GetAssetProofResponse
): Promise<CNFTTransferArgs> {
  if (!assetFields.compression) {
  ...
  const root = new Uint8Array(
    getAddressEncoder().encode(address(proofFields.root))
  );
  const index = assetFields.compression.leaf_id;

  // 2. construct TMetadataArgsArgs
  const tMetadataArgsArgs: TMetadataArgsArgs | null =
    getTMetadataArgsArgs(assetFields);
