...
export function findMetadataArgs(assetFields: DAS.GetAssetResponse): MetadataArgs {
...
function convertAssetFieldsToMetadataArgs(assetFields: DAS.GetAssetResponse): MetadataArgs {
    const {
        compression,
        content,
        royalty,
    ...
    const coll = grouping ? grouping.find((group: DAS.Grouping) => group.group_key === "collection")?.group_value : "";
    ...
    return {
        ...
        tokenStandard: tokenStandard === "Fungible" ? TTokenStandard.Fungible :
            tokenStandard === "NonFungibleEdition" ? TTokenStandard.NonFungibleEdition :
                tokenStandard === "FungibleAsset" ? TTokenStandard.FungibleAsset :
                    tokenStandard === "NonFungible" ? TTokenStandard.NonFungible : null,
        collection: coll ? { key: coll, verified: true } as TCollection : null,
        uses: uses
            ? {
                useMethod: uses.use_method === "Burn" ? TUseMethod.Burn : uses.use_method === "Multiple" ? 1 : TUseMethod.Single,
...
export function metadataArgsToTMetadataArgsArgs(metadataArgs: MetadataArgs): TMetadataArgsArgs {
    const tMetadataArgsArgs: TMetadataArgsArgs = {
        ...
        creatorShares: Buffer.from(metadataArgs.creators.map((creator: TCreator) => (creator.share))),
        creatorVerified: metadataArgs.creators.map((creator: TCreator) => (creator.verified))
    }
