...
export async function fetchMetadata(rpc: Rpc<SolanaRpcApi>, metadataPda: Address): Promise<any> {
    const metadataData = await rpc.getAccountInfo(metadataPda, { encoding: 'base64' }).send().then((result: any) => result.value.data[0]);
