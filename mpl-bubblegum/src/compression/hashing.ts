...
export function computeCompressedNFTHash(
    ...
    const message = Buffer.concat([
        ...
        computeDataHash(metadata, new Uint8Array(getU16Encoder().encode(metadata.sellerFeeBasisPoints))),
        computeCreatorHash(metadata.creators),
    ]);

    return Buffer.from(keccak_256.digest(message));
}
export const computeDataHash = (metadataArgs: MetadataArgs, sellerFeeBasisPointsBuffer: Uint8Array) =>
    Buffer.from(
        keccak_256.digest(
            Buffer.concat([
